...
async function main() {
  ...
  // Clear existing data (only clear what exists in your schema)
  ...
  const regionGameVendors = await prisma.regionGameVendor.createMany({
    data: [
      ...
      // Some inactive relationships for testing
  ...
  // Get created users for transactions
  ...
  // Create transactions one by one to handle any field issues
  console.log('💳 Creating transactions...');

  // XCoin Purchase Transactions (Retailers)
  for (let i = 0; i < retailers.length; i++) {
    const retailer = retailers[i];
    
    // Completed XCoin purchase
    ...
    // Package Purchase Transaction (spending XCoins)
    await prisma.transaction.create({
      data: {
        userId: retailer.id,
  ...
  // Pending XCoin purchases for first 2 retailers
  ...
  // Reseller Bulk Purchase Transactions
  ...
  // Pending bulk purchase for first reseller
  ...
  // Create XCoin transactions for balance tracking
 ...
 const packageTransactions = await prisma.transaction.findMany({
  ...
  for (let i = 0; i < packageTransactions.length; i++) {
    ...
    // Create vendor call for each order
